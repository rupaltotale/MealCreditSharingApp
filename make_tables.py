...
def make_tables():
    ...
    cursor.execute('create table Availability (user_id INT, asking_price FLOAT NOT NULL, location varchar(64), start_time TIME, end_time TIME, \
                    foreign key (user_id) references Users(user_id))')

    cursor.execute('create table Hunger (user_id INT, max_price FLOAT NOT NULL, location varchar(64), start_time TIME, end_time TIME, \
                    foreign key (user_id) references Users(user_id))')
